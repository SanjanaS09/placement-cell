...
const StudentPage = ({ studentName }) => {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      ...
      const fetchedAnnouncements = querySnapshot.docs.map(doc => doc.data());
      setAnnouncements(fetchedAnnouncements);
    };
  ...
  return (
    <div className="student-page">
      ...
      <motion.div className="announcements" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <h2>Announcements</h2>
        <ul>
      ...
      <motion.div className="events" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <h2>Upcoming Events</h2>
        <ul>
          {events.map((event, index) => (
            <li key={index}>
              <strong>{event.name}</strong>: {event.description} <br />
              <em>{event.date} | {event.time} | {event.venue} | Speaker: {event.speaker}</em>
            </li>
          ))}
        </ul>
      </motion.div>
